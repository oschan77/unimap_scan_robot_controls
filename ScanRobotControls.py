...
class ScanRobotControls:
    ...
    def record(self):
        ...
        self.proc_record = subprocess.Popen(
            f"echo {self.password} | sudo -S {cmd_record}",
            shell=True,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        time.sleep(1)  # Give some time for the process to start and get its PID
        self.proc_record.stdin.write(self.password.encode())
        self.proc_record.stdin.flush()
